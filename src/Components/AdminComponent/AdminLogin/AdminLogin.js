     ...
     function AdminLogin(){
        ...
        let history = useHistory();
         ...
         async function login(e){
             const value = await axios.get(`${baseUrl}/admin?name=${admin.admin_name}`); 

            //  console.log(value.data[0].admin_name);
            //  console.log(admin.admin_name);
           console.log('lgin',value.data);

             if(value.data[0].name === admin.admin_name)
             {
                if(value.data[0].password === admin.admin_password){
                    alert("success");
